<template>
    <Head :title="$t('admin.customers.show') + ' ' + $t('admin.menu.customers')"/>
    <div class="row g-2 align-items-center mb-4">
        <div class="col">
            <h2 class="page-title text-capitalize">{{ customer.full_name }} {{ $t('admin.menu.customers') }}</h2>
        </div>
            ...
            <div class="card-body py-5 px-3">
                <div class="datagrid">
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.customers.name') }}</div>
                        <div class="datagrid-content">{{ customer.full_name }}</div>
                    </div>
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.customers.email') }}</div>
                        <div class="datagrid-content">{{ customer.email }}</div>
                    </div>
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.customers.mobile') }}</div>
                        <div class="datagrid-content">{{ customer.mobile ?? '-' }}</div>
                    </div>
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.users.sex') }}</div>
                        <div class="datagrid-content">{{ customer.sex ?? '-' }}</div>
                    </div>
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.customers.birthdate') }}</div>
                        <div class="datagrid-content">{{ customer.birthdate ?? '–' }}</div>
                    </div>
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.customers.nationality') }}</div>
                        <div class="datagrid-content">{{ customer.national?.name ?? '-' }}</div>
                    </div>
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.customers.status') }}</div>
                    ...
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.customers.email_verified') }}</div>
                        <div class="datagrid-content">
                            <span class="badge bg-success-lt" v-if="customer.email_verified_at">
                                {{ $t('admin.customers.verified') }}
                            </span>
                            <span class="badge bg-danger-lt" v-if="!customer.email_verified_at">
                                {{ $t('admin.customers.not_verified') }}
                            </span>
                        </div>
                    </div>
                    <div class="datagrid-item">
                        <div class="datagrid-title">{{ $t('admin.customers.mobile_verified') }}</div>
                        <div class="datagrid-content">
